...
async function connectToWhatsApp() {
    ...
    sock = makeWASocket({
        ...
        getMessage: async (key) => {
            return { conversation: 'Jeeyhosting System Message' };
        }
    });
    ...
    if(!sock.authState.creds.registered && !pairingCodeRequested) {
        ...
        setTimeout(async () => {
            try {
                ...
                let code = await sock.requestPairingCode(phoneNumber, 'JEEYVIPP');
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if(connection === 'close') {
            ...
            console.log("Bot WhatsApp Berhasil Terhubung ke Jeeyhosting API!");
        }
    });
}
connectToWhatsApp();
